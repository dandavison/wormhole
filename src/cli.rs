...
pub fn run(command: Command) -> Result<(), String> {
    ...
    match command {
        ...
        Command::Refresh => {
            let url = format!("{}/project/refresh", client.base_url);
            let resp = ureq::post(&url).call().map_err(map_ureq_error)?;
            render_refresh_progress(resp);
            Ok(())
        }
    }
}

fn render_refresh_progress(resp: ureq::Response) {
    ...
    let spinner_style = ProgressStyle::with_template("{spinner:.cyan} {msg}")
        .unwrap()
        .tick_strings(TICKS);
    let counter_style = ProgressStyle::with_template("{spinner:.cyan} {msg} [{pos}/{len}]")
        .unwrap()
        .tick_strings(TICKS);
    let sub_style = ProgressStyle::with_template("  {spinner:.dim} {msg:.dim}")
        .unwrap()
        .tick_strings(TICKS);

    let multi = MultiProgress::new();
    let mut phase_spinner: Option<ProgressBar> = None;
    let mut header: Option<ProgressBar> = None;
    let mut task_spinners: HashMap<String, ProgressBar> = HashMap::new();
    ...
    for line in reader.lines() {
        ...
        match event {
            Event::Phase { ref name } => {
                let label = match name.as_str() {
                    "worktrees" => "Discovering worktrees",
                    "kv" => "Loading data",
                    _ => name,
                };
                ...
                phase_spinner = Some(pb);
            }
            Event::PhaseDone { .. } => {
                if let Some(pb) = phase_spinner.take() {
                    pb.finish_and_clear();
                }
            }
            Event::CacheStart { total } => {
                let pb = multi.add(ProgressBar::new(total as u64));
                pb.set_style(counter_style.clone());
                pb.enable_steady_tick(tick);
                pb.set_message("Refreshing tasks");
                header = Some(pb);
            }
            Event::TaskStart { ref name } => {
                let pb = if let Some(ref h) = header {
                    multi.insert_after(h, ProgressBar::new_spinner())
                } else {
                ...
                task_spinners.insert(name.clone(), pb);
            }
            Event::TaskDone { ref name } => {
                if let Some(pb) = task_spinners.remove(name) {
                    pb.finish_and_clear();
                }
